...
#[test]
fn wide_transcript_has_run_header_reserved_inspector_and_role_hierarchy() {
    ...
    let buffer = render_buffer(&state, 140, 30);
    assert_eq!(
        cell_at_token(&buffer, "╭").fg,
        Color::Rgb(0x2b, 0x31, 0x39),
        "passive Run panel must not compete with composer focus"
    );
}
...
fn below_default_breakpoint_uses_full_width_single_column_without_run_inspector() {
    for width in [139, 99] {
        ...
        assert!(
            narrow.lines().any(|line| line.starts_with("  You")),
            "width {width} transcript should start at the left edge\n{narrow}"
        );
        assert!(narrow.contains("  Slim"), "width {width}\n{narrow}");
        assert!(
...
fn error_color_is_reserved_for_the_failure_marker() {
    ...
    let error = Color::Rgb(0xf0, 0x71, 0x78);
