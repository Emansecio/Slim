...
fn latest_short_turn_page_fills_from_its_prompt_after_long_history() {
    ...
    assert_eq!(
        question_row, 2,
        "session header and prompt label must page-fill before the body\n{frame}"
    );
...
fn turn_boundary_adds_one_row_only_before_later_user() {
    ...
    let frame = render_at_size(&state, 80, 16);
    let rows = transcript_rows(&frame, 80, 16);
    ...
    assert!(
        ...
        "first user must not have a leading spacer\n{frame}"
    );
    assert!(
        second_question >= 2,
    ...
    assert!(
        ...
        "user label must remain adjacent to its body\n{frame}"
    );
}
