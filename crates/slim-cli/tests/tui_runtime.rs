...
#[test]
fn tui_plan_runs_the_read_only_loop_instead_of_approval_required() {
    let root = std::env::temp_dir().join(format!("slim-tui-plan-{}", std::process::id()));
    std::fs::create_dir_all(&root).expect("workspace");
    let source = root.join("fixture.txt");
    std::fs::write(&source, "plan fixture\n").expect("fixture");
    ...
    let server = thread::spawn(move || {
        for turn in 0..2 {
            ...
            if turn == 0 {
                ...
                assert!(!request.contains("\"name\":\"write\""));
            ...
            if turn == 0 {
                ...
                stream
                    .write_all(b"data: {\"choices\":[{\"delta\":{\"content\":\"plan complete\"}}]}\n\ndata: {\"usage\":{\"prompt_tokens\":5,\"completion_tokens\":4}}\n\ndata: {\"choices\":[{\"delta\":{},\"finish_reason\":\"stop\"}]}\n\ndata: [DONE]\n\n")
                    .expect("final completion");
            }
        }
    });
    ...
    .expect("plan tui turn");
    server.join().expect("server");

    assert!(
        !events.iter().any(|event| matches!(
            event,
            UiEvent::Notification { message } if message == "approval_required"
        )),
        "Plan TUI must not toast approval_required"
    );
    assert!(events.iter().any(|event| matches!(
        event,
        UiEvent::ToolStarted { name, .. } if name == "read"
