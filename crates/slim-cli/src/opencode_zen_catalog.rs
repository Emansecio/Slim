...
use slim_core::provider::{zen_model, zen_models, OPENCODE_ZEN_MODELS_URL};
...
impl fmt::Display for CatalogError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(match self {
            Self::Oversized => "OpenCode Zen catalog exceeds its bound",
            Self::InvalidSchema => "OpenCode Zen catalog schema is invalid",
            Self::InvalidModelId => "OpenCode Zen catalog model id is invalid",
            Self::DuplicateModelId => "OpenCode Zen catalog contains a duplicate model id",
            Self::Network => "OpenCode Zen catalog request failed",
            Self::UnsafeCache => "OpenCode Zen catalog cache path is unsafe",
            Self::CacheIo => "OpenCode Zen catalog cache I/O failed",
        })
    }
}
...
#[derive(Clone)]
pub struct OpenCodeZenCatalog {
    url: String,
    cache_path: PathBuf,
    client: reqwest::Client,
}

impl OpenCodeZenCatalog {
    pub fn production() -> Result<Self, CatalogError> {
        let cache_path = if let Some(path) = std::env::var_os("SLIM_OPENCODE_ZEN_MODELS_FILE") {
            if path.is_empty() {
            ...
            PathBuf::from(profile)
                .join(".slim")
                .join("opencode-zen-models.json")
        };
        Self::at(OPENCODE_ZEN_MODELS_URL, cache_path)
    }
...
/// The live catalog lists every Zen model (paid included); the selector keeps
/// only ids with verified local metadata — the zero-cost set.
